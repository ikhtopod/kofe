...
Mesh* CreateSphere(const Color& color) {
    ...
    mesh->GetTransform().SetScale(glm::vec3 { 1.5f });
...
DirectionalLight::DirectionalLight() :
    DirectionalLight { DEFAULT_AMBIENT, DEFAULT_DEFFUSE, DEFAULT_SPECULAR } {}

DirectionalLight::DirectionalLight(const Color& color) :
    DirectionalLight { color, DEFAULT_AMBIENT, DEFAULT_DEFFUSE, DEFAULT_SPECULAR } {}

DirectionalLight::DirectionalLight(float ambient, float diffuse, float specular) :
    DirectionalLight { Color::WHITE, ambient, diffuse, specular } {}

DirectionalLight::DirectionalLight(const Color& color, float ambient,
...
float DirectionalLight::GetDiffuse() const {
    return m_diffuse;
}
...
Color DirectionalLight::GetAmbientColor() const {
    return Color { static_cast<glm::vec3>(m_color) * m_ambient };
}

Color DirectionalLight::GetDiffuseColor() const {
    return Color { static_cast<glm::vec3>(m_color) * m_diffuse };
}

Color DirectionalLight::GetSpecularColor() const {
